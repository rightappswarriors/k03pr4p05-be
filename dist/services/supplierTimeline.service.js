...
function statusForDelivery(status) {
    if (status === 'DELIVERED')
        return 'SUCCESS';
    if (status === 'FAILED')
        return 'ERROR';
...
function statusForMandate(status) {
    if (status === 'COMPLETED' || status === 'SETTLED' || status === 'FUNDED')
        return 'SUCCESS';
...
function statusForLedger(status, type) {
    if (status === 'REVERSED')
        return 'ERROR';
    if (status === 'HELD')
        return 'WARNING';
...
function inDateRange(createdAt, range) {
    if (!range)
        return true;
    if (range.start && createdAt < range.start)
        return false;
    if (range.end && createdAt > range.end)
        return false;
    return true;
}
function groupLabel(createdAt, now = new Date()) {
...
export async function getSupplierOrderTimeline(prisma, filters) {
    const { supplierOrgId, search, status, eventTypes, dateRange, sort = 'NEWEST', limit = 30, offset = 0, } = filters;
    const take = Math.max(25, Math.min((limit ?? 30) * 3, 150));
    const orderBy = { createdAt: sort === 'OLDEST' ? 'asc' : 'desc' };
    const enabledTypes = new Set(eventTypes?.length ? eventTypes : Object.keys(EVENT_CONFIG));
    const events = [];
    const [purchaseOrders, deliveries, wallet, mandateTransactions, mandateOffers, supplierItems, notifications, auditLogs] = await Promise.all([
    ...
    for (const log of auditLogs) {
        const eventType = log.pageKey?.toLowerCase().includes('organization') ? 'ORGANIZATION' : 'SYSTEM';
