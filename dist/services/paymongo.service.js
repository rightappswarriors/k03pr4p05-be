 ...
 */
export async function attachPaymentIntent(paymentIntentId, paymentMethodId, clientKey, secret_key) {
    const base64key = Buffer.from(`${secret_key}`).toString();
    try {
        const response = await axios.post(`${API_BASE}/payment_intents/${paymentIntentId}/attach`, {
            headers: {
                accept: "application/json",
                "content-type": "application/json",
                authorization: `Basic ${base64key}`,
