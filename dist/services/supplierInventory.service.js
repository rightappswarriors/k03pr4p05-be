import { SupplierInventoryMovementType, SupplierStockBatchStatus, SupplierIncomingStatus } from '@prisma/client';
async function postMovement(tx, input) {
    if (input.quantity <= 0)
        throw new Error('VALIDATION: movement quantity must be > 0');
    ...
    if (newAvailable < -0.0001)
        throw new Error(`INSUFFICIENT_STOCK: available stock cannot go negative for item ${item.id}`);
    if (newReserved < -0.0001)
        throw new Error(`INVALID_RESERVATION: reserved stock cannot go negative for item ${item.id}`);
...
async function consumeFifoBatches(tx, supplierItemId, quantity, warehouseId) {
    ...
    if (remaining > 0.0001) {
        throw new Error(`INSUFFICIENT_BATCH_STOCK: ${remaining} unit(s) of item ${supplierItemId} have no batch coverage. ` +
...
// ────────────────────────────────────────────────────────────────────────
export async function getInventoryValuation(prisma, supplierItemId) {
...
async function computeAverageCost(tx, supplierItemId) {
    const v = await getInventoryValuation(tx, supplierItemId);
    return v.totalQty > 0 ? v.averageCost : null;
}
export async function receiveStock(prisma, input) {
    if (input.quantity <= 0)
        throw new Error('VALIDATION: quantity must be > 0');
    if (input.unitCost < 0)
        throw new Error('VALIDATION: unitCost cannot be negative');
...
// fulfilled/delivered.
// ────────────────────────────────────────────────────────────────────────
export async function reserveStock(prisma, input) {
    if (input.quantity <= 0)
        throw new Error('VALIDATION: quantity must be > 0');
    return prisma.$transaction((tx) => postMovement(tx, {
        supplierItemId: input.supplierItemId,
...
export async function releaseReservation(prisma, input) {
    if (input.quantity <= 0)
        throw new Error('VALIDATION: quantity must be > 0');
    return prisma.$transaction((tx) => postMovement(tx, {
        supplierItemId: input.supplierItemId,
...
// ────────────────────────────────────────────────────────────────────────
export async function fulfillReservedStock(prisma, input) {
    if (input.quantity <= 0)
        throw new Error('VALIDATION: quantity must be > 0');
    return prisma.$transaction(async (tx) => {
...
export async function sellDirectStock(prisma, input) {
    if (input.quantity <= 0)
        throw new Error('VALIDATION: quantity must be > 0');
    return prisma.$transaction(async (tx) => {
        const consumed = await consumeFifoBatches(tx, input.supplierItemId, input.quantity, input.warehouseId);
...
// ────────────────────────────────────────────────────────────────────────
export async function adjustStock(prisma, input) {
    if (input.delta === 0)
        throw new Error('VALIDATION: adjustment delta cannot be zero');
    return prisma.$transaction(async (tx) => {
        if (input.delta > 0) {
...
// ────────────────────────────────────────────────────────────────────────
export async function markDamaged(prisma, input) {
    if (input.quantity <= 0)
        throw new Error('VALIDATION: quantity must be > 0');
    return prisma.$transaction(async (tx) => {
        const consumed = await consumeFifoBatches(tx, input.supplierItemId, input.quantity, input.warehouseId);
...
export async function markReturned(prisma, input) {
    if (input.quantity <= 0)
        throw new Error('VALIDATION: quantity must be > 0');
    return prisma.$transaction((tx) => postMovement(tx, {
        supplierItemId: input.supplierItemId,
...
export async function restockReturnedItem(prisma, input) {
    if (input.quantity <= 0)
        throw new Error('VALIDATION: quantity must be > 0');
    return prisma.$transaction(async (tx) => {
        const item = await tx.supplierItem.findUniqueOrThrow({ where: { id: input.supplierItemId } });
        if (item.returnedQty < input.quantity)
            throw new Error('VALIDATION: cannot restock more than currently returned quantity');
        const batch = await tx.supplierStockBatch.create({
            data: {
                supplierItemId: input.supplierItemId,
                warehouseId: input.warehouseId ?? null,
...
// ────────────────────────────────────────────────────────────────────────
export async function transferStock(prisma, input) {
    if (input.quantity <= 0)
        throw new Error('VALIDATION: quantity must be > 0');
    if (input.fromWarehouseId === input.toWarehouseId)
        throw new Error('VALIDATION: source and destination warehouse must differ');
...
// ────────────────────────────────────────────────────────────────────────
export async function logIncomingStock(prisma, input) {
    if (input.expectedQty <= 0)
        throw new Error('VALIDATION: expectedQty must be > 0');
    return prisma.$transaction(async (tx) => {
...
export async function cancelIncomingStock(prisma, incomingStockId) {
    return prisma.$transaction(async (tx) => {
        const incoming = await tx.supplierIncomingStock.findUniqueOrThrow({ where: { id: incomingStockId } });
        if (incoming.status !== SupplierIncomingStatus.PENDING)
            throw new Error('VALIDATION: only PENDING incoming stock can be cancelled');
...
export async function receiveIncomingStock(prisma, input) {
    const incoming = await prisma.supplierIncomingStock.findUniqueOrThrow({ where: { id: input.incomingStockId } });
    if (incoming.status !== SupplierIncomingStatus.PENDING)
        throw new Error('VALIDATION: this incoming stock has already been received or cancelled');
...
// ────────────────────────────────────────────────────────────────────────
export async function reconcileInventoryRollups(prisma, supplierItemId) {
    return prisma.$transaction(async (tx) => {
        ...
        return {
            ...
            driftDetected: before.availableQty !== after.availableQty ||
...
// ────────────────────────────────────────────────────────────────────────
export async function backfillOpeningBalances(prisma) {
    ...
    for (const item of items) {
        ...
        if (batchCount > 0)
            continue; // already has batches, skip
...
// ────────────────────────────────────────────────────────────────────────
export async function getInventoryForecast(prisma, supplierItemId, trailingDays = 30) {
    ...
    const suggestedReorderDate = daysRemaining !== null && reorderLevel !== null && avgDailyConsumption
...
// ────────────────────────────────────────────────────────────────────────
export async function getInventoryAnalytics(prisma, supplierItemId) {
    const item = await prisma.supplierItem.findUniqueOrThrow({ where: { id: supplierItemId } });
    ...
    for (const b of batches) {
        ...
        if (ageDays <= 30)
            agingBuckets.fresh += b.remainingQty;
        else if (ageDays <= 60)
            agingBuckets.aging += b.remainingQty;
        else if (ageDays <= 90)
            agingBuckets.old += b.remainingQty;
        else
            agingBuckets.stale += b.remainingQty;
    }
    const avgDaysInStock = batches.length > 0
...
// ────────────────────────────────────────────────────────────────────────
export async function getSupplierInventoryDashboard(prisma, orgId) {
