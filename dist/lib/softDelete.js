...
function hasExplicitDeletedAt(where) {
    ...
    return ["AND", "OR", "NOT"].some((key) => {
        ...
        if (Array.isArray(value))
            return value.some(hasExplicitDeletedAt);
        return hasExplicitDeletedAt(value);
    });
}
function applyWhereFilter(args) {
    args.where = args.where ?? {};
    if (!hasExplicitDeletedAt(args.where)) {
        args.where.deletedAt = null;
    }
}
function relationModel(parentModel, fieldName) {
    const model = dmmfModels.get(parentModel);
    const field = model?.fields?.find((candidate) => candidate.name === fieldName);
    return field?.kind === "object" ? field.type : undefined;
}
function isListRelation(parentModel, fieldName) {
    const model = dmmfModels.get(parentModel);
    const field = model?.fields?.find((candidate) => candidate.name === fieldName);
    return Boolean(field?.isList);
}
function applyNestedRelationFilters(parentModel, node) {
...
export async function restoreById(prisma, model, id) {
...
export async function restoreMany(prisma, model, where) {
    if (!SOFT_DELETE_MODELS.has(model)) {
        throw new Error(`Model ${model} is not configured for soft delete.`);
    }
    const delegate = prisma[model.charAt(0).toLowerCase() + model.slice(1)];
