...
function cartesian(arrays) {
    return arrays.reduce((acc, arr) => acc.flatMap((existing) => arr.map((item) => [...existing, item])), [[]]);
}
// ─── resolver ─────────────────────────────────────────────────────────────────
    ...
    definition(t) {
        ...
        t.nonNull.field('generateVariants', {
            ...
            resolve: async (_, { supplierItemId, basePrice, baseCost }, ctx) => {
                ...
                const optionIdArrays = groups.map((g) => g.options.map((o) => o.id));
                ...
                const existingKeys = new Set(existing.map((v) => v.variantValues
                    .map((vv) => vv.optionId)
                    .sort()
                    .join(',')));
                const created = [];
