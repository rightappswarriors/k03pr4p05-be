    ...
    definition(t) {
        t.field('createSummaryRow', {
            ...
            resolve: async (_, { orgId, accountTitleId, vatTypeId, centerId, subCenterId, itemId, itemName, costLines, costInputAmount, costInputVatInclusive, sellingPriceInput, sellingPriceVatInclusive, opExPct, description, itemCode, }, ctx) => {
                ...
                const costInput = costLinesArray.length > 0
                    ? costLinesArray.reduce((sum, line) => sum + (Number(line?.amount) || 0), 0)
                    : costInputAmount ?? 0;
        ...
        t.field('updateSummaryRow', {
            ...
            resolve: async (_, { id, accountTitleId, vatTypeId, centerId, subCenterId, itemId, itemName, costLines, costInputAmount, costInputVatInclusive, sellingPriceInput, sellingPriceVatInclusive, opExPct, description, itemCode, }, ctx) => {
                ...
                const rate = vatType?.rate ?? 0;
                const existingCostLines = (existing.costLines ?? []);
                const incomingCostLines = costLines !== undefined && Array.isArray(costLines)
                    ? costLines
                    : existingCostLines;
                const costInput = incomingCostLines.length > 0
                    ? incomingCostLines.reduce((sum, line) => sum + (Number(line?.amount) || 0), 0)
                    : costInputAmount !== undefined
                        ? costInputAmount
                        : existing.amount;
                const costVatInclusive = costInputVatInclusive !== undefined
                ...
                const vatInput = costVatInclusive
                    ? costInput - baseCost
                    : baseCost * rate;
                const revenueInput = sellingPriceInput !== undefined
                    ? sellingPriceInput
                    : existing.sellingPrice + (existing.vatOutput ?? 0);
                const revenueVatInclusive = sellingPriceVatInclusive !== undefined
                ...
                const vatOutput = revenueVatInclusive
                    ...
                    : sellingPrice * rate;
                const finalOpExPct = opExPct !== undefined ? opExPct : existing.opExPct;
