...
async function generateOrderNumber(prisma, orgId) {
...
function sanitizeExtraCharges(extraCharges) {
    return (extraCharges ?? []).map((charge) => {
        const label = String(charge.label ?? "").trim();
        const amount = Number(charge.amount ?? 0);
        if (!label)
            throw new Error("Extra charge label is required.");
...
function initialStatusFor(orderMode) {
    return orderMode === "WALK_IN" ? "PROCESSING" : "PENDING";
}
function nextStatusFor(orderMode, status) {
    const flow = {
...
function labelOrderMode(orderMode) {
    if (orderMode === "PICK_UP")
        return "Pick-up";
...
async function recalculateExtraTotals(tx, salesOrderId) {
    ...
    const extraChargesTotal = charges.reduce((sum, charge) => sum + Number(charge.amount ?? 0), 0);
