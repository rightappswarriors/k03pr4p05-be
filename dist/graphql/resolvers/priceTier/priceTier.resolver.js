...
// HELPERS
// ─────────────────────────────────────────────────────────────
function computeMargin(sellingPrice, cost) {
    if (!sellingPrice)
        return 0;
    return ((sellingPrice - cost) / sellingPrice) * 100;
}
function computeMarkup(sellingPrice, cost) {
    if (!cost)
        return 0;
...
export const pricingDashboard = queryField('pricingDashboard', {
    ...
    resolve: async (_root, { catalogId }, ctx) => {
        ...
        const margins = items.map((i) => computeMargin(i.unitPrice, i.currentCost));
        const activeCount = items.length;
        const avgPrice = activeCount
            ? items.reduce((sum, i) => sum + i.unitPrice, 0) / activeCount
            : 0;
...
export const pricingList = queryField('pricingList', {
    ...
    resolve: async (_root, { catalogId, page, pageSize, filter }, ctx) => {
        ...
        if (filter?.minMargin != null) {
            items = items.filter((i) => i.margin >= filter.minMargin);
        }
        if (filter?.maxMargin != null) {
            items = items.filter((i) => i.margin <= filter.maxMargin);
        }
...
export const pricingAnalytics = queryField('pricingAnalytics', {
    ...
    resolve: async (_root, { supplierItemId }, ctx) => {
        ...
        const prices = priceTrend.map((p) => p.newPrice);
