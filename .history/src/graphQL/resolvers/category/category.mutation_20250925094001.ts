  ...
  definition(t) {
    ...
    t.nonNull.field("updateCategory", {
      ...
      async resolve(_, { id, name }, ctx) {
        ...
        await requireOwnership(ctx, "Category", id);
        if (!id || !name ) {
          throw new Error("Requires Name and Category")
        }
        try {
          return await categoryService.updateCategoryById(Number(id), name)
        } catch (error) {
    ...
    t.nonNull.field("deleteCategory", {
      ...
      async resolve(_, { id }, ctx) {
        ...
        await requireOwnership(ctx, "Category", id);
        if (!id ) {
          throw new Error("Select Category to delete")
        }
        try {
          return await categoryService.deleteCategory(Number(id))
        } catch (error) {
