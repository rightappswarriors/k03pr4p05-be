import { extendType, nonNull, intArg, objectType, nullable, stringArg, enumType } from "nexus";
...
export const sortEnum = enumType({
  name: "SortEnum",
  members: ["asc", "desc"]
})
  ...
  definition(t) {
    t.nonNull.list.nonNull.field("getItems", {
      ...
      async resolve(_, {query, size}, ctx){
        ...
        try {
          return await itemService.getItems(query, size)
        } catch (error) {
