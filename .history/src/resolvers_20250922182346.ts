...
export const resolvers = {
  ...
  User: {
     branchesOwned: (parent: any) =>
       mockdata.branches.filter((b) => b.ownerId === parent.id),
     storesOwned: (parent: any) =>
          mockdata.stores.filter((s) => s.ownerId === parent.id),
   },
   Branch: {
     owner: (parent: any) => mockdata.users.find((u) => u.id === parent.ownerId),
     stores: (parent: any) => mockdata.stores.filter((s) => s.branchId === parent.id),
   },
   Store: {
     owner: (parent: any) => mockdata.users.find((u) => u.id === parent.ownerId),
     branch: (parent: any) => mockdata.branches.find((b) => b.id === parent.branchId),
   },
};
