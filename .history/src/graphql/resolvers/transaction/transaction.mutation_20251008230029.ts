...
export const TransactionMutation = extendType({
  ...
  definition(t) {
    ...
    t.field("initiatePayment", {
      ...
      resolve: async (_, {  args: transactionData }, ctx) => {
        requireAuth(ctx);
        requireRole(ctx, ["ADMIN", "MANAGER", "CASHIER", "STAFF"]);
        try {
          return await transactionService.initiatePayment(transactionData);
        } catch (error) {
          console.error("Error upon making transaction through", transactionData)
        }
      },
    });
  },
