...
export const AddItemToInventory = inputObjectType({
  ...
  definition(t) {
    ...
    t.field("createInventoryItem", {
      ...
      async resolve(_, { inventoryId, items }, ctx) {
        ...
        const outletId = ctx.prisma.inventory.findFirst({
        ...
        if (!outletId) {
          throw new Error("Cant find Inventory")
        }
        await requireOwnership(ctx, "outlet", outletId)
        try {
          return await inventoryService.createInventoryItem(items, Number(inventoryId));
        } catch (error) {
