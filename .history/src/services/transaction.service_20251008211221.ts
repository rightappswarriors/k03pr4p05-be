...
import * as paymongoService  from "./paymongo.service";
...
export const processTransaction = async (transactionData, itemsSold) => {
  ...
  if (transactionData.paymentMethod !== "CASH") {
    const owner = await prisma.outlet.findUnique({
    ...
    const userKeys = await prisma.paymongoAPIKeys.findUnique({
      where: { ownerId: owner.branch.ownerId },
      select: { public_key: true, secret_key: true },
    });

    const public_key = decrypt(userKeys.public_key);
    const secret_key = decrypt(userKeys.secret_key);
    const paymenMethodData= await paymongoService.createPaymentMethod({
      paymentType:transactionData.paymentType,
      secret_key,
    })
    const paymentIntent = paymongoService.createPaymentIntent({
