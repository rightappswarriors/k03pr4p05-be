...
export const updateAPIKeyById = async(id: number, {data}) => {
  let  public_key, secret_key;
  if (!data.public_key){
    public_key = encrypt(data.public_key)
  } 
  if (!data.secret_key){
    secret_key = encrypt(data.secret_key)
  }  
  const updatedKey = await prisma.paymongoAPIKeys.update({
    where: {id},
    data: {
