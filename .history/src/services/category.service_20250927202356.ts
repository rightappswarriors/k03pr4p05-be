...
export const getAllCategories = async (query, orderBy, pageSize, page=1) => {
...
export const getItemsByCategoryId = async (id) => {
  ...
  const categoryWithItems = await prisma.category.findUnique({
    where: { id },
    select: {
      Item: true, // Selects the items related to this category.
    },
  });

  // Return the array of items, or null if the category wasn't found.
  return categoryWithItems?.Item || null;
};
