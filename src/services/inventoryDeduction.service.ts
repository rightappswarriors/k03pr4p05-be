...
async function deductInventoryItem(tx: TxClient, inventoryItemId: number, quantity: number, options?: { unitId?: number | null }) {
...
export async function deductSalesOrderInventory(tx: TxClient, salesOrderId: string) {
  ...
  if (!order.outlet?.inventory) throw new Error("Sales order outlet inventory not found.");
