...
export interface DashboardFilterInput {
  /** ISO string — defaults to start of today */
  startDate?: string;
  /** ISO string — defaults to end of today */
  endDate?: string;
}
...
export interface DashboardStats {
  ...
  // Renamed from "Today" → scoped to the selected date range
  ...
  // Growth — scoped to the selected date range
...
export interface AdminDashboardData {
  ...
  /** ISO strings echoed back so the frontend knows what range was applied */
...
export class AdminDashboardService {
   ...
   */
  async getDashboard(filter?: DashboardFilterInput): Promise<AdminDashboardData> {
    const now = new Date();
  ...
  // async getOrdersLast30Days() { ... }
  // async getUserGrowthChart() { ... }
  // async getOrganizationGrowthChart() { ... }
  // async getSubscriptionStatistics() { ... }
  // async getAuditLogFeed(limit: number) { ... }
  // async getSystemHealth() { ... }
  // async getFeatureUsage() { ... }
  // async getTopOrganizationsBySales(limit: number) { ... }
}
