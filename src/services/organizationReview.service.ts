...
import type { ReviewAggregate } from './supplierItemReview.service.js'
...
async function hasVerifiedTransaction(prisma: PrismaClient, organizationId: number, reviewerOrgId?: number | null) {
  if (!reviewerOrgId) return false
...
export async function getOrganizationReviewAggregate(
  prisma: PrismaClient,
...
export async function listOrganizationReviews(prisma: PrismaClient, organizationId: number) {
...
export async function deleteOrganizationReview(prisma: PrismaClient, id: string, reviewerOrgId: number) {
  const review = await prisma.organizationReview.findFirst({
