...
type AnalyticsOrder = {
    ...
    outlet?: { branchId?: number | null } | null;
    items?: any[];
};
...
const SALES_ORDER_REVENUE_STATUSES = ['RECEIVED', 'COMPLETED'] as const;
const KOMPRA_ORDER_REVENUE_STATUS = 'received';
const TRANSACTION_REVENUE_STATUSES = ['PAID', 'SYNCED', 'COMPLETED'] as const;

function normalizeOrder(
    order: any,
    createdAt: Date | string,
    source: AnalyticsOrder['source'],
): AnalyticsOrder {
    return {
        ...order,
...
function salesOrderSource(order: any): AnalyticsOrder['source'] {
    return order.orderMode === 'WALK_IN' ? 'sales_order_walk_in' : 'sales_order_other';
}

function lineQuantity(line: any): number {
...
function orderBelongsToBranch(order: AnalyticsOrder, branchId: number, outletIds: Set<number>): boolean {
    if (order.branchId === branchId || order.outlet?.branchId === branchId) return true;
    return !!order.outletId && outletIds.has(order.outletId);
}

function addItemsToMap(itemMap: Record<number, any>, orders: AnalyticsOrder[]) {
...
function addPrevUnits(prevUnits: Record<number, number>, orders: AnalyticsOrder[]) {
    for (const order of orders) {
        for (const line of order.items ?? []) {
...
function buildSourceBreakdown(orders: AnalyticsOrder[]) {
    const sourceOrder: AnalyticsOrder['source'][] = [
        'pos',
        'sales_order_walk_in',
        'sales_order_other',
        'kompra',
    ];
    const buckets = sourceOrder.reduce(
        (map, source) => {
            map[source] = {
                source,
                totalRevenue: 0,
        ...
        {} as Record<AnalyticsOrder['source'], {
            source: AnalyticsOrder['source'];
            totalRevenue: number;
            totalCost: number;
            grossProfit: number;
    ...
    for (const order of orders) {
        ...
        bucket.unitsSold += (order.items ?? []).reduce(
            (sum: number, line: any) => sum + lineQuantity(line),
            0,
        );
    }
    ...
    definition(t) {
        ...
        t.nonNull.field('getSalesAnalytics', {
            ...
            async resolve(_, { preset, startDate, endDate }, ctx) {
                ...
                const [currentTx, prevTx, currentSalesOrders, prevSalesOrders, currentKompraOrders, prevKompraOrders, branches] = await Promise.all([
                    ctx.prisma.transaction.findMany({
                        where: {
                            ...
                            status: { in: TRANSACTION_REVENUE_STATUSES as any },
                        },
                        include: {
                    ...
                    ctx.prisma.salesOrder.findMany({
                        where: {
                            ...
                            status: { in: SALES_ORDER_REVENUE_STATUSES as any },
                        },
                        include: {
                            items: { include: { item: { include: { orgCategory: true } } } },
                            outlet: { select: { branchId: true } },
                ...
                const normalizedCurrentTx = currentTx.map((order: any) => normalizeOrder(order, order.createdAt, 'pos'));
                const normalizedPrevTx = prevTx.map((order: any) => normalizeOrder(order, order.createdAt, 'pos'));
                const normalizedCurrentSalesOrders = currentSalesOrders.map((order: any) => normalizeOrder(order, order.date, salesOrderSource(order)));
                const normalizedPrevSalesOrders = prevSalesOrders.map((order: any) => normalizeOrder(order, order.date, salesOrderSource(order)));
                const normalizedCurrentKompraOrders = currentKompraOrders.map((order: any) => normalizeOrder(order, order.createdAt, 'kompra'));
                const normalizedPrevKompraOrders = prevKompraOrders.map((order: any) => normalizeOrder(order, order.createdAt, 'kompra'));
                ...
                const totalRevenue = allCurrentOrders.reduce(
                    (sum, order) => sum + Number(order.total ?? 0),
                    0,
                );
                const prevRevenue = allPrevOrders.reduce(
                    (sum, order) => sum + Number(order.total ?? 0),
                    0,
                );

                const totalCost = allCurrentOrders.reduce((sum, order) => sum + orderCost(order), 0);
                const totalItemsSold = allCurrentOrders.reduce(
                    (sum, order) =>
                        sum +
                        (order.items ?? []).reduce(
                            (itemSum: number, line: any) => itemSum + lineQuantity(line),
                            0,
                        ),
                    0,
                );
                ...
                for (const branch of branches) {
                    const branchOutletIds = new Set<number>(branch.outlets.map((o: any) => Number(o.id)));
                    const bCurrent = allCurrentOrders.filter((order) =>
                        orderBelongsToBranch(order, branch.id, branchOutletIds),
                    );
                    const bPrev = allPrevOrders.filter((order) =>
                        orderBelongsToBranch(order, branch.id, branchOutletIds),
                    );

                    const bRevenue = bCurrent.reduce(
                        (sum: number, order: any) => sum + Number(order.total ?? 0),
                        0,
                    );
                    const bPrevRevenue = bPrev.reduce(
                        (sum: number, order: any) => sum + Number(order.total ?? 0),
                        0,
                    );

                    const bCost = bCurrent.reduce((sum: number, order) => sum + orderCost(order), 0);
                ...
                const itemMap: Record<number, ItemAggregate> = {};
        ...
        t.nonNull.field('getBranchAnalytics', {
            ...
            async resolve(_, { branchId, preset, startDate, endDate }, ctx) {
                ...
                const outletIds = branch.outlets.map((o: any) => o.id);
                ...
                const currentOrders = [
                    ...currentTx.map((order: any) => normalizeOrder(order, order.createdAt, 'pos')),
                    ...currentSalesOrders.map((order: any) => normalizeOrder(order, order.date, salesOrderSource(order))),
                    ...currentKompraOrders.map((order: any) => normalizeOrder(order, order.createdAt, 'kompra')),
                ];
                const prevOrders = [
                    ...prevTx.map((order: any) => normalizeOrder(order, order.createdAt, 'pos')),
                    ...prevSalesOrders.map((order: any) => normalizeOrder(order, order.date, salesOrderSource(order))),
                    ...prevKompraOrders.map((order: any) => normalizeOrder(order, order.createdAt, 'kompra')),
                ];

                const revenue = currentOrders.reduce((s: number, t: any) => s + Number(t.total ?? 0), 0);
                const prevRevenue = prevOrders.reduce((s: number, t: any) => s + Number(t.total ?? 0), 0);
                const cost = currentOrders.reduce((s: number, order) => s + orderCost(order), 0);
        ...
        t.nonNull.field('getItemAnalyticsPaginated', {
            ...
            async resolve(_, { preset, startDate, endDate, take = 20, page = 1, search, section = 'top' }, ctx) {
                ...
                const pageSlice = all.slice(
                    (clampedPage - 1) * safeTake,
                    clampedPage * safeTake,
                );

                const toPerf = (item: typeof all[0], rank: number): ItemPerformance => {
                    const profit = item.revenue - item.cost;
                    const margin = item.revenue > 0 ? (profit / item.revenue) * 100 : 0;
        ...
        t.nullable.field("getDashboardOrderStats", {
            ...
            resolve: async (_, args, ctx) => {
                ...
                for (const order of allOrders) {
                    ...
                    if (s === "PROCESSING" || s === "SHIPPED" || s === "OUT_FOR_DELIVERY" || s === "READY_FOR_PICKUP") processingOrders += 1;
...
interface ItemPerformance {
    ...
    posSalesCount: number; posUnitsSold: number;
    salesOrderWalkInSalesCount: number; salesOrderWalkInUnitsSold: number;
    kompraOrderCount: number; kompraUnitsSold: number;
...
interface SourceBreakdown {
    source: string; totalRevenue: number; totalCost: number; grossProfit: number;
    totalOrders: number; unitsSold: number;
}
