  ...
  definition(t) {
    ...
    t.field("deleteRestockSchedule", {
      ...
      async resolve(_, { id }, ctx) {
        ...
        await Promise.all(cycles.map((c: any) => removeCycleJob(c.id).catch(() => { })));
    ...
    t.field("toggleRestockSchedule", {
      ...
      async resolve(_, { id }, ctx) {
        ...
        if (newActive) {
          ...
          await Promise.all(cycles.map((c: any) => registerCycleJob(c).catch(() => { })));
        }
    ...
    t.field("createRestockCycle", {
      ...
      async resolve(_, { data }, ctx) {
        ...
        await registerCycleJob(cycle).catch(err => {
          if (process.env.NODE_ENV === "development") console.error(`Failed to register cycle job for cycle ${cycle.id}:`, err);
        });
    ...
    t.field("updateRestockCycle", {
      ...
      async resolve(_, { id, data }, ctx) {
        ...
        await registerCycleJob(updated).catch(err => {
          if (process.env.NODE_ENV === "development") console.error(`Failed to re-register cycle job for cycle ${id}:`, err);
        });
