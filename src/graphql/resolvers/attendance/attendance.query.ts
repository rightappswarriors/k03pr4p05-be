  ...
  definition(t) {
    ...
    t.field('myPerformanceSummary', {
      ...
      resolve: async (_, { from, to }, ctx) => {
        requireAuth(ctx)
        return attendanceService.getMyPerformanceSummary(
          ctx.user.userId,
          new Date(from),
          new Date(to)
        )
    ...
    t.field('todayAttendanceByOrg', {
      ...
      resolve: async (_, { page, limit, role }, ctx) => {
        ...
        return attendanceService.getTodayAttendanceByOrg(
          ctx.user.orgId,
          page ?? 1,
          limit ?? 20,
          role ?? undefined
    ...
    t.field('userAttendanceHistory', {
      ...
      resolve: async (_, { userId, page, limit }, ctx) => {
        requireAuth(ctx)
        requireRole(ctx, ['OWNER'])
        return attendanceService.getUserAttendanceHistory(
          ctx.user.orgId,
          userId,
          page ?? 1,
          limit ?? 20
    ...
    t.field('userPerformanceSummary', {
      ...
      resolve: async (_, { userId, from, to }, ctx) => {
        requireAuth(ctx)
        requireRole(ctx, ['OWNER'])
        return attendanceService.getUserPerformanceSummary(
          ctx.user.orgId,
          userId,
          new Date(from),
          new Date(to)
