  ...
  definition(t) {
    ...
    t.nonNull.list.nonNull.field('getKompraCOrdersForManagement', {
      ...
      resolve: async (_root, args, ctx) => {
        ...
        const statusList = args.status?.split(',').map((s: any) => s.trim()).filter(Boolean)
        return ctx.prisma.kompraCOrder.findMany({
          where: {
            outlet: { orgId },
...
export const KompraCMutation = extendType({
  ...
  definition(t) {
    ...
    t.nonNull.field('placeKompraOrder', {
      ...
      resolve: async (_root, { customerId, input }, ctx) => {
        const inventoryItemIds = input.items.map((i: any) => i.inventoryItemId)
        const liveItems = await ctx.prisma.inventoryItems.findMany({
          where: { id: { in: inventoryItemIds } },
          include: { item: true },
        })
        type LiveItem = (typeof liveItems)[number]
        const liveMap = new Map<number, LiveItem>(liveItems.map((i: any) => [i.id, i]))
        ...
        const subtotal = orderItems.reduce((sum: any, i: any) => sum + i.subtotal, 0)
        ...
        const breakdown = await computeScPwdBreakdown(
          ...
          orderItems.map((item: any) => ({ itemId: item.itemId, quantity: item.quantity, price: item.priceSnapshot, priceAtSale: item.priceSnapshot })),
          weeklyBnpcState,
        )
    ...
    t.nonNull.field('confirmKompraOrder', {
      ...
      resolve: async (_root, args, ctx) => {
        if (process.env.NODE_ENV === 'development') console.log(`${LOG_PREFIX} confirmKompraOrder — orderId:`, args.orderId)
        ...
        try {
          ...
          if (process.env.NODE_ENV === "development") console.log(`${LOG_PREFIX} confirmKompraOrder — success. status:`, result.status)
          return result
        } catch (error) {
          if (process.env.NODE_ENV === "development") console.error(`${LOG_PREFIX} confirmKompraOrder — ERROR:`, error)
          throw error
        }
      },
    })
    ...
    t.nonNull.field('markKompraOrderPacked', {
      ...
      resolve: async (_root, args, ctx) => {
        if (process.env.NODE_ENV === 'development') console.log(`${LOG_PREFIX} markKompraOrderPacked — orderId:`, args.orderId)
        requireKompraManagementAccess(ctx)
        PAGE_PERMISSIONS.kompraOrder.edit(ctx)
        try {
          ...
          if (process.env.NODE_ENV === "development") console.log(`${LOG_PREFIX} markKompraOrderPacked — success. status:`, result.status, '| packedAt:', result.packedAt)
          return result
        } catch (error) {
          if (process.env.NODE_ENV === "development") console.error(`${LOG_PREFIX} markKompraOrderPacked — ERROR:`, error)
          throw error
        }
      },
    })
    ...
    t.nonNull.field('assignKompraOrderRider', {
      ...
      resolve: async (_root, args, ctx) => {
        if (process.env.NODE_ENV === "development") console.log(`${LOG_PREFIX} assignKompraOrderRider — orderId:`, args.orderId, '| rider:', args.riderName)
        requireKompraManagementAccess(ctx)
        PAGE_PERMISSIONS.kompraOrder.edit(ctx)
        ...
        try {
          ...
          if (process.env.NODE_ENV === "development") console.log(`${LOG_PREFIX} assignKompraOrderRider — success. status:`, result.status, '| shippedAt:', result.shippedAt)
          return result
        } catch (error) {
          if (process.env.NODE_ENV === "development") console.error(`${LOG_PREFIX} assignKompraOrderRider — ERROR:`, error)
          throw error
        }
      },
    })
    ...
    t.nonNull.field('markOrderInDelivery', {
      ...
      resolve: async (_root, args, ctx) => {
        ...
        try {
          ...
          if (process.env.NODE_ENV === "development") console.error(`${LOG_PREFIX} markOrderInDelivery — ERROR:`, error)
          throw error
        }
      },
    })
    ...
    t.nonNull.field('confirmOrderReceived', {
      ...
      resolve: async (_root, { orderId, customerId }, ctx) => {
        ...
        try {
          ...
          if (process.env.NODE_ENV === "development") console.error(`${LOG_PREFIX} confirmOrderReceived — ERROR:`, error)
          throw error
        }
      },
    })
    ...
    t.nonNull.field('markKompraOrderDelivered', {
      ...
      resolve: async (_root, { orderId }, ctx) => {
        if (process.env.NODE_ENV === "development") console.log(`${LOG_PREFIX} markKompraOrderDelivered — orderId:`, orderId)
        requireKompraManagementAccess(ctx)
        PAGE_PERMISSIONS.kompraOrder.edit(ctx)
        try {
          const result = await ctx.prisma.$transaction(async (tx: any) => {
          ...
          if (process.env.NODE_ENV === "development") console.log(`${LOG_PREFIX} markKompraOrderDelivered — success.`)
          return result
        } catch (error) {
          if (process.env.NODE_ENV === "development") console.error(`${LOG_PREFIX} markKompraOrderDelivered — ERROR:`, error)
          throw error
        }
      },
    })
    ...
    t.nonNull.field('cancelKompraOrder', {
      ...
      resolve: async (_root, args, ctx) => {
        if (process.env.NODE_ENV === "development") console.log(`${LOG_PREFIX} cancelKompraOrder — orderId:`, args.orderId, '| reason:', args.reason)
        requireKompraManagementAccess(ctx)
        PAGE_PERMISSIONS.kompraOrder.edit(ctx)
        ...
        try {
          ...
          if (process.env.NODE_ENV === "development") console.log(`${LOG_PREFIX} cancelKompraOrder — success. cancelledAt:`, result.cancelledAt)
          return result
        } catch (error) {
          if (process.env.NODE_ENV === "development") console.error(`${LOG_PREFIX} cancelKompraOrder — ERROR:`, error)
          throw error
        }
      },
    })
