  ...
  definition(t) {
    ...
    t.nonNull.field("item", {
      ...
      resolve: (parent, _, ctx) =>
        ctx.prisma.restockScheduleItem.findUnique({ where: { id: parent.id } }).item(),
    });
  },
});
...
export const RestockScheduleType = objectType({
  ...
  definition(t) {
    ...
    t.field("branch", {
      type: "Branch",
      resolve: (parent, _, ctx) =>
        parent.branchId ? ctx.prisma.branch.findUnique({ where: { id: parent.branchId } }) : null,
    });
    t.field("outlet", {
      type: "Outlet",
      resolve: (parent, _, ctx) =>
        parent.outletId ? ctx.prisma.outlet.findUnique({ where: { id: parent.outletId } }) : null,
    });
    t.nonNull.list.nonNull.field("scheduleItems", {
      type: "RestockScheduleItem",
      resolve: (parent, _, ctx) =>
        ctx.prisma.restockSchedule.findUnique({ where: { id: parent.id } }).scheduleItems({
          include: { item: true },
        }),
    });
    ...
    t.nonNull.list.nonNull.field("cycles", {
      type: "RestockCycle",
      resolve: (parent, _, ctx) =>
        ctx.prisma.restockSchedule.findUnique({ where: { id: parent.id } }).cycles({
...
export const RestockCycleItemType = objectType({
  ...
  definition(t) {
    ...
    t.nonNull.field("item", {
      type: "Item",
      resolve: (parent, _, ctx) =>
        ctx.prisma.restockCycleItem.findUnique({ where: { id: parent.id } }).item(),
    });
  },
});
...
export const RestockCycleType = objectType({
  ...
  definition(t) {
    ...
    t.nonNull.list.nonNull.field("cycleItems", {
      ...
      resolve: (parent, _, ctx) =>
        ctx.prisma.restockCycle.findUnique({ where: { id: parent.id } }).cycleItems({
          include: { item: true },
        }),
    });
  },
