...
export const SummaryRow = objectType({
  ...
  definition(t) {
    ...
    t.nullable.field('item', {      // ← was nonNull, now nullable
      type: 'Item',
      resolve: (parent, _, ctx) => {
        if (!parent.itemId) return null  // ← guard added
    ...
    t.nonNull.field('vatType', {
      type: 'VatType',
      resolve: (parent, _, ctx) =>
        ctx.prisma.summaryRow.findUnique({ where: { id: parent.id } }).vatType()
    })
    t.nonNull.field('accountTitle', {
      type: 'AccountTitle',
      resolve: (parent, _, ctx) =>
        ctx.prisma.summaryRow.findUnique({ where: { id: parent.id } }).accountTitle()
    })
    t.nonNull.field('center', {
      type: 'Center',
      resolve: (parent, _, ctx) =>
        ctx.prisma.summaryRow.findUnique({ where: { id: parent.id } }).center()
    })
    t.nonNull.field('subCenter', {
      type: 'SubCenter',
      resolve: (parent, _, ctx) =>
        ctx.prisma.summaryRow.findUnique({ where: { id: parent.id } }).subCenter()
    })
    t.nonNull.field('org', {
      type: 'Organization',
      resolve: (parent, _, ctx) =>
        ctx.prisma.summaryRow.findUnique({ where: { id: parent.id } }).org()
    })
    t.nullable.field('user', {
      type: 'User',
      resolve: (parent, _, ctx) => {
        if (!parent.userId) return null  // ← guard added
