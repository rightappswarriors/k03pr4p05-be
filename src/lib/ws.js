...
export function initWebSocket(server) {
  ...
  wss.on("connection", function connection(ws, request) {
    ...
    let claims;
    try { claims = jwt.verify(token, process.env.JWT_SECRET || "token"); }
    catch { ws.close(1008, "Unauthorized"); return; }
    if (!claims?.userId) { ws.close(1008, "Unauthorized"); return; }
    ...
    ws.on("message", (message) => {
      ...
      if ((evt === "conversation:leave" || evt === "leave") && data.conversationId) {
        ...
        if (process.env.NODE_ENV === "development") console.log(`[PortalWebSocket] LEAVE ${room} user:${userIdKey}`);
        return;
      }
      ...
      if (["typing:start", "typing:stop", "conversation:typing", "conversation:messageRead", "presence:update"].includes(evt)) {
        ...
        emit(
          room,
          "conversation:typing",
          { ...payload, senderOrgId: client.orgId, senderId: client.userId }
        );
      }
    });
    ...
    ws.on("close", () => {
      ...
      if (process.env.NODE_ENV === "development") console.log(`[PortalWebSocket] DISCONNECTED user:${userIdKey} clientId:${clientId}`);
    });
...
function emit(room, event, payload) {
  ...
  if (publisher?.isOpen) {
    ...
    void publisher.publish(
      "kompra:realtime",
      JSON.stringify({ source: bridgeId, room, event, payload })
    );
  }
}
...
export function sendToUser(userId, event, payload) {
  ...
  if (process.env.NODE_ENV === "development") console.log(`[PortalWebSocket] sendToUser user:${userIdKey} event=${event}`);
...
export function sendToConversation(conversationId, event, payload) {
  if (process.env.NODE_ENV === "development") console.log(`[PortalWebSocket] sendToConversation conversation:${conversationId} event=${event}`);
