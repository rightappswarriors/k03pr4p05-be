...
export interface OutletSearchResult {
  ...
  matchCount:     number;   // how many of the requested items are in stock
  totalItems:     number;   // total items the customer searched for
  matchLabel:     string;   // "3/3 items", "2/3 items"
  matchScore:     number;   // 0-1, used for sorting (1 = perfect match + closest)
  matchedItems: {
    itemId:       number;
    itemName:     string;
    price:        number;
    stockQty:     number;
    available:    boolean;  // false if stock < quantityWanted
  }[];
  missingItems: {
    itemId:       number;
    itemName:     string;
...
export async function findNearestOutletsWithItems(
  customerLat: number,
  customerLng: number,
  ...
  const outletMap = new Map<number, typeof indexRows>();
  ...
  results.sort((a, b) => {
    ...
    if (b.matchCount === b.totalItems && a.matchCount < a.totalItems) return 1;
    if (a.matchCount === a.totalItems && b.matchCount < b.totalItems) return -1;
...
export async function refreshOutletItemSearchIndex(outletId: number) {
  ...
  await Promise.all(
    inventoryItems.map((inv) =>
      prisma.outletItemSearchIndex.upsert({
        where: {
          outletId_itemId: {
